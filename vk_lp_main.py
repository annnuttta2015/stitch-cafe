...
async def handle_top(user_id: int, peer_id: int) -> None:
    """Handle /top command (admins only): full stats in *личку* администратору.

    Аналог поведения телеграм-бота:
    - статистика уходит в личные сообщения админу;
    - если команда вызвана из беседы, в беседу отправляется короткое подтверждение.
    """
    ...
    if not rows:
        # Пытаемся отправить админу в личку сообщение о пустой базе
        try:
            send_message(user_id, EMPTY_DB)
            if peer_id != user_id:
                send_message(peer_id, TOP_SENT_DM)
        except Exception:
            if peer_id != user_id:
                name_mention = format_vk_user_mention(user_id, get_user_first_name(user_id))
                send_message(peer_id, TOP_DM_FAIL.format(name=name_mention))
        return
    ...
    # Пытаемся отправить полную статистику в личку админу
    try:
